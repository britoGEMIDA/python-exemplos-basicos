...
import sys # Importa a biblioteca sys para acessar variáveis e funções específicas do sistemas

def resource_path(relative_path):
    """ Obtém o caminho absoluto para o recurso, funciona para dev e para o PyInstaller """
    try:
        # PyInstaller cria um diretótio temporário e armazena o caminho em _MEIPASS
...
class Calculadora:
    def __init__(self):
        ...
        self.janela = ttk.Window(themename="darkly") # Cria a janela principal usando ttkbootstra
        ...
        self.fonte_display = ('Roboto', 36) # Fontedo display
        ...
        for i, linha in enumerate(self.botoes): # Itera sobre as linhas de botões
            for j, texto in enumerate(linha): # Itera sobre os botões em cada linha
                ...
                botao = ttk.Button(
                    ...
                    command=partial(self.interpretar_botao, texto) # Define o comando para o botão, usando partial para passar o textodo botão
                )
        ...
        self.frame_imagem = ttk.Frame(self.janela) # Cria um frame para a magem SENAI
        ...
        label_imagem.image = imagem_tk # Matém uma referência para a imagem (necessário para evitar que a imagem seja destruída pelo garbage collector)
    ...
    def interpretar_botao(self, valor):
        """ Interpreta o botão pressionado e atualiza display """
